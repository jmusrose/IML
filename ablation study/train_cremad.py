...
from cmi_fgm import register_feature_gradient_hooks, register_split_linear_weight_hook
from ablation_fgm import AblationFGMState, add_fgm_arguments, compute_fgm_signal
...
from models import AVBaseline, AudioBaseline, VisualBaseline
from training import clone_model_state_dict, prepare_run_output_dir
...
def build_fgm_state(args: argparse.Namespace) -> AblationFGMState | None:
    if not getattr(args, "fgm", False):
        return None
    if args.modality != "av":
        return None
    feature_path = bool(getattr(args, "fgm_feature_path", True))
    classifier_path = bool(getattr(args, "fgm_classifier_path", True))
    if not feature_path and not classifier_path:
        return None
    return AblationFGMState(
        modalities=("audio", "visual"),
        strength=args.fgm_lambda,
        temperature=args.fgm_tau,
        momentum=args.fgm_momentum,
        warmup_steps=args.fgm_warmup_steps,
        signal_name=getattr(args, "fgm_signal", "delta"),
        use_relative=getattr(args, "fgm_use_relative", True),
        use_absolute=getattr(args, "fgm_use_absolute", True),
...
def forward_and_losses(
    ...
    modality: str,
    criterion: nn.Module,
    fgm_state: AblationFGMState | None = None,
    ...
    if modality == "av" and hasattr(model, "forward_with_modal_logits"):
        ...
        if fgm_state is not None:
            ...
            if fgm_state.classifier_path and isinstance(getattr(model, "classifier", None), nn.Linear):
                handles.append(
                    register_split_linear_weight_hook(
                        model.classifier,
                        split_sizes=(outputs["audio_feature"].shape[1], outputs["visual_feature"].shape[1]),
                        modalities=("audio", "visual"),
                        coefficients=coefficients,
                    )
                )
            signal = compute_fgm_signal(
                fgm_state.signal_name,
                fusion_per_sample,
                audio_per_sample,
                visual_per_sample,
                losses["audio_acc"],
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    add_fgm_arguments(parser, warmup_default=12)
