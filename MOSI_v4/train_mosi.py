...
from MOSI_v4.datasets import MOSIDataset, load_mosi_splits, mosi_collate_fn
...
def forward_and_losses(
    ...
    if hasattr(model, "forward_with_modal_predictions"):
        ...
        outputs = model.forward_with_modal_predictions(
            ...
            batch["vision_mask"],
            batch["audio_mask"],
            detach_probe_features=resolved_detach,
        )
...
def train_one_epoch(
    ...
    for raw_batch in iterator:
        ...
        predictions, losses, handles = forward_and_losses(
            model,
            batch,
            criterion,
            fgm_state=fgm_state,
...
def run_training(args: argparse.Namespace) -> dict[str, float]:
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = train_one_epoch(
            ...
            fgm_state=fgm_state,
            text_loss_weight=getattr(args, "text_loss_weight", 1.0),
            vision_loss_weight=getattr(args, "vision_loss_weight", 1.0),
            audio_loss_weight=getattr(args, "audio_loss_weight", 1.0),
            detach_probe_features=getattr(args, "detach_probe_features", True),
        )
...
def build_arg_parser(
    ...
    parser.add_argument("--vision-loss-weight", type=float, default=1.0)
    parser.add_argument("--audio-loss-weight", type=float, default=1.0)
    parser.add_argument(
        "--detach-probe-features",
        dest="detach_probe_features",
        action="store_true",
        help="Detach modality features before probe heads outside warmup.",
    )
    parser.add_argument(
        "--no-detach-probe-features",
        dest="detach_probe_features",
        action="store_false",
        help="Let probe losses update modality encoders for the whole run.",
    )
    parser.set_defaults(detach_probe_features=True)
    return parser
