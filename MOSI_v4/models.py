...
class MOSIRegressionModel(nn.Module):
    ...
    def forward_with_modal_predictions(
        ...
        text_probe_feature = text_feature.detach() if detach_probe_features else text_feature
        vision_probe_feature = vision_feature.detach() if detach_probe_features else vision_feature
        ...
        return {
            ...
            "text_prediction": self.text_probe(text_probe_feature).squeeze(-1),
            "vision_prediction": self.vision_probe(vision_probe_feature).squeeze(-1),
            "audio_prediction": self.audio_probe(audio_probe_feature).squeeze(-1),
