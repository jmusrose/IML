...
class AVBaseline(nn.Module):
    ...
    def forward_with_modal_logits(
        ...
        visual_feature = self.extract_visual_feature(visual)
        fusion_feature = torch.cat([audio_feature, visual_feature], dim=1)
        audio_weight = self.classifier.weight[:, :512]
        visual_weight = self.classifier.weight[:, 512:]
        half_bias = self.classifier.bias * 0.5 if self.classifier.bias is not None else None
        audio_logits = F.linear(audio_feature, audio_weight, half_bias)
        visual_logits = F.linear(visual_feature, visual_weight, half_bias)
