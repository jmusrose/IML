...
def run_training(args: argparse.Namespace) -> dict[str, float]:
    ...
    if sizes["train"] == 0 or sizes["test"] == 0:
        raise ValueError(f"Empty NYUD2 split: {sizes}")
    ...
    print(f"NYUD2 classes: {args.num_classes}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train RGB_v4 on NYUD2 RGB scene images.")
    parser.add_argument("--data-root", type=str, default="dataset/nyud2_trainvaltest")
    parser.add_argument("--output-dir", type=str, default="runs/rgb_v4_nyud2")
