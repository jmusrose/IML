...
class LinearFusionHead(nn.Module):
    def __init__(self, num_classes: int, classifier: nn.Linear | None = None) -> None:
        super().__init__()
        self.classifier = classifier if classifier is not None else nn.Linear(1024, num_classes)
    ...
    def forward(
        ...
        visual_logits: torch.Tensor | None = None,
    ) -> torch.Tensor:
        del audio_logits, visual_logits
...
class LogitFusionHead(nn.Module):
    def __init__(self, num_classes: int, classifier: nn.Linear | None = None) -> None:
        super().__init__()
        self.classifier = classifier if classifier is not None else nn.Linear(num_classes * 2, num_classes)
    ...
    def forward(
        ...
        if audio_logits is None or visual_logits is None:
            raise ValueError("LogitFusionHead requires audio_logits and visual_logits.")
        return self.classifier(torch.cat([audio_logits, visual_logits], dim=1))


class TransEncoderFusionHead(nn.Module):
    def __init__(
        self,
        num_classes: int,
        feature_dim: int = 512,
    ...
    def forward(
        ...
        del audio_logits, visual_logits
        tokens = torch.stack([audio_feature, visual_feature], dim=1)
...
def build_fusion_head(
    ...
    if fusion_head == "linear":
        ...
        return LinearFusionHead(num_classes, classifier), classifier
    if fusion_head == "logit_fusion":
        classifier = nn.Linear(num_classes * 2, num_classes)
        return LogitFusionHead(num_classes, classifier), classifier
    if fusion_head == "transencoder":
        head = TransEncoderFusionHead(num_classes)
        return head, head.classifier
...
class AVBaseline(nn.Module):
    def __init__(
        ...
        self.fusion_head, self.classifier = build_fusion_head(fusion_head, num_classes)
    ...
    def forward_with_modal_logits(
        ...
        audio_logits = self.audio_probe(audio_feature)
        visual_logits = self.visual_probe(visual_feature)
        logits = self.fusion_head(audio_feature, visual_feature, audio_logits, visual_logits)
        return {
            "logits": logits,
